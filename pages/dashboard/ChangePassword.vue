<template>
  <Modal v-if="true">
    <div v-motion :initial="{ opacity: 0, scale: 0.9 }" :visible="{ opacity: 1, scale: 1 }" class="w-[500px] relative h-fite mx-auto bg-white  rounded-[4px] shadow-md px-5  py-10 sm:p-16 space-y-7
  ">
      ...
      <h2 class="text-center text-[#211658] text-3xl font-medium">Change Password</h2>
      <form :onSubmit="handleSubmit(handleAddAdmin)" class="space-y-5 md:space-y-7 ">
        <div class="space-y-2 pb-4  md:space-y-3">
          <CustomInput input-type="text" v-model="newPassword" v-bind="newPasswordProps" label="New Password"
            name="newPassword" />
          <CustomInput input-type="text" v-model="confirmPassword" v-bind="confirmPasswordProps" label="Confirm Password"
