...
const allUsers = ref<{ email: string; name: string }[]>([]);
...
const getAllUsers = async () => {
  const response = await axios.get(
    `https://api-staging.mealtrips.com/api/admin/users/all/customer`,
    {
      headers: authHeader(),
    },
  );
  const data = response.data.data;
  const e =data.map((vendor: { email: string; fullname: string }) => {
    return {
      email: vendor.email,
...
const handleSendBroadcast = async (
  ...
  const usersEmails = form_data.emails?.map((email) => email.email);
