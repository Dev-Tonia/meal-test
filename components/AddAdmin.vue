<template>
  ...
  ">
    <Icon @click="closeModal" role="button" name="ci:close-big" color="#292D32" size="34px"
      class="absolute top-10 right-10 bg-[#A1B1CC] p-1.5 cursor-pointer rounded-full" />
    <h2 class="text-center text-[#211658] text-3xl font-medium">{{ isEdit ? 'Edit Admin' : 'Add Admin' }}</h2>
    <form :onSubmit="handleSubmit(handleAddAdmin)" class="space-y-5 md:space-y-10 ">
      <div class="space-y-2 pb-4  md:space-y-3">
        <CustomInput :value="selectedAdminUser.firstname" input-type="text" v-model="firstName" v-bind="firstNameProps"
          label="First Name" name="firstName" />
        <CustomInput :value="selectedAdminUser.lastname" input-type="text" v-model="lastName" v-bind="lastNameProps"
          label="Last Name" name="lastName" />
        <CustomInput :value="selectedAdminUser.email" input-type="email" v-model="email" v-bind="emailProps" label="Email"
          name="email" />
        <CustomSelect :value="selectedAdminUser.role" label="Role" v-model="role" v-bind="roleProps" name="role" />
...
const { loading, addAdminStatus, selectedAdminUser } = storeToRefs(useGlobalStore())
...
const handleAddAdmin = async (data: subAdminPayloadInterface) => {
  ...
  try {
    if (props.isEdit) {
      const res = await axios.put(`https://api-staging.mealtrips.com/api/admin/admin-users/edit-admin`, {
