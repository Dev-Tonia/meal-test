...
<template>
  <div class="w-full relative pb-1.5">
    ...
    <div class="rounded-[5px] border relative items-center cursor-pointer px-2.5  gap-3">
      <div @click="handleClick" class="absolute w-full bg-transparent z-10 h-12 left-0" />
      <div>
        <slot></slot>
      </div>
      ...
      <span class="absolute top-1/2 -translate-y-1/2 right-3">
        <Icon v-motion :initial="{ y: 4 }" :enter="{ y: 0 }" v-if="!openDropdown" name="lucide:chevron-down" color="gray"
          size="18px" />
        <Icon v-motion :initial="{ y: 4 }" :enter="{ y: 0 }" v-else name="lucide:chevron-up" color="gray" size="18px" />
      </span>
